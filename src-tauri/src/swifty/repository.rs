...
impl SwiftyRepository {
    pub fn from_repo_json(url: String) -> Result<SwiftyRepository> {
        ...
        let repo_map: HashMap<String, Value> =
            serde_json::from_slice(repo_json.as_slice()).unwrap();
        ...
        let icon_image_checksum: String =
            repo_map["iconImageChecksum"].as_str().unwrap().to_string();
        let repo_image_path: String = repo_map["repoImagePath"].as_str().unwrap().to_string();
        let repo_image_checksum: String =
            repo_map["repoImageChecksum"].as_str().unwrap().to_string();
        ...
        let required_mods: Vec<Mod> =
            Mod::from_repo_json(repo_map["requiredMods"].as_array().unwrap())?;
        let optional_mods: Vec<Mod> =
            Mod::from_repo_json(repo_map["optionalMods"].as_array().unwrap())?;
        let servers: Vec<Server> = Server::from_repo_json(repo_map["servers"].as_array().unwrap())?;

        return Ok(SwiftyRepository {
            repo_name,
            icon_image_path,
            icon_image_checksum,
            repo_image_path,
