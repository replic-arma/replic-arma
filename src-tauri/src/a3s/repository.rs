...
use crate::a3s::classes::{AutoConfig, Changelogs, SyncTreeDirectory, ServerInfo, Events};
use super::utils::{fetch_meta_file, fetch, unzip, FromJavaObject};
use crate::repository::{Repository, Modset, ModsetMod, GameServer, DownloadServer, File, DownloadServerOptions};
...
impl A3SRepository {
    pub fn from_auto_config(url: String) -> Result<A3SRepository, Box<dyn std::error::Error>> {
        ...
        return Ok(A3SRepository{auto_config, changelogs, sync, server_info, events});
    }
    ...
    pub fn to_repository(&self) -> Result<Repository, Box<dyn std::error::Error>> {
        ...
        for server in self.auto_config.favorite_servers.iter() {
            game_servers.push(GameServer{
                ...
                modset: if server.modset_name == "" { None } else { Some(server.modset_name.clone()) },
            });
        }

        let download_server = DownloadServer{
            url: self.auto_config.protocol.to_url()?,
            options: DownloadServerOptions{ max_connections: self.server_info.number_of_connections }
        };
